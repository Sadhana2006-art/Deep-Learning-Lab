...
\section*{18. Plots with Inference}

\subsection*{18.1 Sample Images}
...
\subsection*{18.2 Training and Validation Accuracy}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Training_Val_Accuracy.png}
\caption{Training and Validation Accuracy}
\end{figure}

\textbf{Interpretation:}

There was steady improvement in training accuracy from about 76.5\% to 94.0\%, showing that the model has learned well. But there were no improvements in validation accuracy, which was at 81-82\% even after the first few epochs. This is a case of overfitting.

\subsection*{18.3 Training and Validation Loss}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Training_Val_Loss.png}
\caption{Training and Validation Loss}
\end{figure}

\textbf{Interpretation:}

Training loss kept decreasing from around 0.76 to 0.17, indicating optimization of the model. On the other hand, the validation loss kept on decreasing but began increasing after the early epochs, suggesting that the model is overfitting to the training data set.

\subsection*{18.4 Confusion Matrix}
...
Confusion matrix shows that the model is capable of producing good accuracy because of the presence of correct predictions for most samples on the diagonal. The classes Ship (973), Truck (966), and Automobile (955) show very good recognition accuracy, whereas there is some confusion among visually close classes like Cat and Dog.

\subsection*{18.5 Misclassified Images}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Misclassified_Images.png}
\caption{Misclassified Images}
\end{figure}

\textbf{Interpretation:}

The misclassified pictures prove that the network is having trouble recognizing images of classes with similar visual properties, like Deer-Cat, Deer-Dog, and Automobile-Truck. The factors which affect misclassification are poor resolution of the image, background interference, and similarity of objects.
...
LeNet-5 & 83,126 & 54.45 & 77.5\\

\hline

AlexNet & 36,925,322 & 71.19 & 343.1\\

\hline

GoogleNet & 22,066,346 & 69.58 & 296.1\\

\hline

ResNet50 & 23,851,274 & 86.38 & 392.7\\
...
AlexNet has been regarded as a breakthrough because it has proven that deep CNNs can be very effective in performing large-scale image classification. This model uses several convolutional layers, Rectified Linear Unit (ReLU), dropout, and GPU training, which enhance the performance of deep learning models.

\item \textbf{Why does VGG16 use only $3\times3$ convolution filters?}

The reason behind the use of $3 \times 3$ convolutional filters by VGG16 is that by combining smaller filters into a stack, a larger receptive field is achieved using a fewer number of parameters compared to one large filter.

\item \textbf{Explain the advantages of the Inception module.}

The Inception module utilizes convolutions with various filter dimensions, such as $1\times1$, $3\times3$, and $5\times5$, in addition to pooling. This way, the network is able to extract features of varying spatial dimensions. The $1\times1$ convolutions further serve the purpose of minimizing computational load and number of parameters.

\item \textbf{What is the purpose of residual learning?}

The reason behind the use of residual learning is that it makes the training of very deep neural networks much more convenient. In this case, instead of training the model for direct learning of the function or mapping, residual learning is used to learn the difference between the input and output in each block.

\item \textbf{Differentiate LeNet and ResNet.}

LeNet is one of the earliest and shallow CNNs that were developed to recognize handwritten digits. The network consists of a few convolutional and pooling layers.

ResNet is one of the deep CNNs which utilizes residual blocks with shortcuts. These enable ResNet to build very deep networks without the issues such as vanishing gradients.

\item \textbf{What is Transfer Learning?}

Transfer Learning is a concept in which the knowledge gained from a model in one particular dataset or problem is transferred to another problem. For instance, a CNN can be trained using a large dataset such as the ImageNet dataset, and this will act as a feature extractor in another image classification problem.

\item \textbf{Why is fine tuning required?}

It is essential to fine-tune the model for making necessary adjustments based on the new data set. Initially, the classification layers are trained, and then some of the pretrained layers can be made trainable by applying a low learning rate.

\item \textbf{Explain the difference between dilated convolution and transpose convolution.}

With dilated convolution, there will be spaces left between the elements of the kernel, enabling the convolution operation to span a larger receptive field by not requiring an increase in kernel parameters.

A transpose convolution is applied to upscale the spatial dimensions of the feature maps. It is applied in areas like image generation, segmentation, and decoding networks.

\item \textbf{Why do pretrained models converge faster?}

Training of pretrained models is fast since it already has meaningful features derived from a huge amount of data. The model does not have to learn from scratch features like edges and texture; instead, it uses pre-existing meaningful weights and adjusts them according to the task at hand.

\item \textbf{Compare the computational complexity of LeNet and ResNet.}

LeNet has an architecture that is comparatively simple, since it has fewer layers and parameters; hence, it uses less computational power and takes less time to train. ResNet has numerous layers and parameters, and thus, it needs more computational resources.

Nevertheless, due to the existence of the residual connections in ResNet, it is possible to have a deep network which is effectively trained. Therefore, LeNet is computationally cheaper, whereas ResNet offers more representational power.
